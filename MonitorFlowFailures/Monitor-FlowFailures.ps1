...
# Store functions as strings for parallel execution
$GetAllPagesFromApiString = ${Function:Get-AllPagesFromApi}.ToString()
$CheckFlowRunInDataverseString = ${Function:Check-FlowRunInDataverse}.ToString()
$AddFlowRunToDataverseString = ${Function:Add-FlowRunToDataverse}.ToString()
...
foreach ($EnvironmentId in $EnvironmentIds) {
    ...
    try {
        ...
        # Process each flow in parallel (5 at a time)
        $results = $sortedFlows | ForEach-Object -ThrottleLimit 5 -Parallel {
            $flow = $_
            $EnvironmentId = $using:EnvironmentId
            $TimeThreshold = $using:TimeThreshold
            $headers = $using:headers
            $DataverseHeaders = $using:DataverseHeaders
            $Prefix = $using:Prefix
            $DataverseTableName = $using:DataverseTableName
            $DataverseUrl = $using:DataverseUrl
            
            # Recreate functions in parallel scope from strings
            ${Function:Get-AllPagesFromApi} = $using:GetAllPagesFromApiString
            ${Function:Check-FlowRunInDataverse} = $using:CheckFlowRunInDataverseString
            ${Function:Add-FlowRunToDataverse} = $using:AddFlowRunToDataverseString
            ...
            # Collect messages instead of writing them immediately
            $flowResult.Messages.Add("  Checking flow: $($flow.properties.displayName) ($($flow.name))")
            ...
            try {
                ...
                # Collect debug info
                $flowResult.Messages.Add("    API returned $($allFlowRuns.Count) runs total")
                if ($allFlowRuns.Count -gt 0) {
                    $flowResult.Messages.Add("    Sample run times and statuses:")
                    $allFlowRuns | Select-Object -First 3 | ForEach-Object {
                        $flowResult.Messages.Add("      $($_.properties.startTime) - $($_.properties.status)")
                    }
                }
                ...
                if ($allFlowRuns.Count -gt 0) {
                    ...
                    $flowResult.Messages.Add("    Total runs in time window: $($allFlowRuns.Count) ($($statusStats -join ', '))")
                } else {
                    $flowResult.Messages.Add("    Total runs in time window: 0")
                }
                ...
                if ($recentFailures.Count -gt 0) {
                    $flowResult.Messages.Add("    Found $($recentFailures.Count) recent failures")
                    $flowResult.TotalFailures = $recentFailures.Count
                    
                    foreach ($failure in $recentFailures) {
                        $flowResult.Messages.Add("      Processing failure: $($failure.name)")
                        ...
                        if (-not $existsInDataverse) {
                            ...
                            if ($added) {
                                $flowResult.NewFailures++
                                $flowResult.Messages.Add("Added flow run failure record: $($failure.name)")
                            }
                        } else {
                            $flowResult.Messages.Add("      Flow run already exists in Dataverse: $($failure.name)")
                        }
                    }
                }
            }
        ...
        $sortedResults = $results | Sort-Object FlowName
        foreach ($result in $sortedResults) {
            # Display main flow info
            Write-Host $result.Messages[0] -ForegroundColor Gray
            
            # Display run statistics and debug info
            for ($i = 1; $i -lt $result.Messages.Count; $i++) {
                $message = $result.Messages[$i]
                if ($message -like "*Total runs in time window*") {
                    Write-Host $message -ForegroundColor Cyan
                } elseif ($message -like "*Found * recent failures*") {
                    Write-Host $message -ForegroundColor Yellow
                } elseif ($message -like "*Processing failure*") {
                    Write-Host $message -ForegroundColor Yellow
                } elseif ($message -like "*Added flow run failure record*") {
        ...
        $results | ForEach-Object {
            $TotalFailuresProcessed += $_.TotalFailures
            $NewFailuresAdded += $_.NewFailures
        }
    }
    catch {
